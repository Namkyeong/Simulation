...
void  list_check(int option, int list);
void  timing(void);
...
float generate1(int stream);
float generate2(int stream);
float generate3(int stream);
...
/* Check a record from list "list" and copy attributes into transfer.
...
float generate1(int stream) /* 1인 그룹 => weibull distribution with lambda=501.58 k=0.89876 */
{	
    return (501.58 * pow(-log(lcgrand(stream)), 1.0/0.89876))/60.0;
}

float generate2(int stream) /* 2인 그룹 =>  2parameter log logistic distribution with alpha=1.6152 beta=163.13*/
{	
    return (163.13 * pow(lcgrand(stream)/(1-lcgrand(stream)), 1.0/1.6152))/60.0;
}

float generate3(int stream) /* 3인 그룹 => frechet distribution with alpha=1.4704 beta=390.16 gamma=-112.21*/
{	
    return (-112.21 + 390.16*(1.0/pow(-log(lcgrand(stream)), 1.0/1.4704)))/60.0;
}

float generate_service(int stream) /* service 분포 => gumbel max distribution with sigma=326.04 mu=955.59*/
{	
	float v,a,b;
	a = exp(-exp(-(600-955.59)/326.03));
	b = exp(-exp(-(3600-955.59)/326.03));
	v = a + (b - a) * lcgrand(stream);
    return (326.03 * (-log(-log(v))) + 955.59) / 60.0;
}
